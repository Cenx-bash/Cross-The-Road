...
namespace Term
{
    ...
    void beep_hit()    { std::cout << "\a"; std::this_thread::sleep_for(std::chrono::milliseconds(40)); std::cout << "\a"; std::cout.flush(); }
    void beep_death()  { for(int i=0;i<3;i++){ std::cout<<"\a"; std::cout.flush(); std::this_thread::sleep_for(std::chrono::milliseconds(80)); } }
...
static const int PLAYER_LANE    = 3;
...
static const int CAR_COLORS[][2] = {
    {196,  52}, {202,  58}, {226,  100}, {46,   22},
    {51,   23}, {201,  90}, {255, 238}, {21,   17},
};
static const int NUM_CAR_COLORS = 8;

Lane* makeLane(int idx)
{
    int rel = ((idx % 15) + 15) % 15;
    LaneType type;
    if      (rel == 0 || rel == 14)                 type = SAFE;
    else if (rel == 1 || rel == 2 || rel == 13)     type = GRASS;
    else if (rel >= 3 && rel <= 5)                  type = ROAD;
    else if (rel == 6)                               type = GRASS;
    else if (rel >= 7 && rel <= 8)                  type = WATER;
    else if (rel == 9)                               type = GRASS;
    else if (rel >= 10 && rel <= 12)                type = ROAD;
    else                                             type = GRASS;
    ...
    if (type == ROAD)
    {
        ...
        {
            Obstacle o;
            o.x = pos; o.width = carW; o.speed = spd; o.vtype = 0;
    ...
    else if (type == WATER)
    {
        ...
        {
            Obstacle o;
            o.x = pos; o.width = logW; o.speed = spd; o.vtype = 1;
...
{
    p.lives--;
    if (p.lives <= 0) { p.alive = false; return true; }
    p.invincible = true;
    p.invincFrames = 45;
    p.hitFlash = true;
    return false;
...
void physTick(World& w, Player& p, bool& soundHit)
{
    ...
    if (cur && cur->type == WATER)
    {
        ...
        if (log) {
            ...
            p.px += drift;
            if (p.px < 0.0 || p.px >= BW) {
                if (!p.invincible) { p.hitFlash = true; loseLife(p); soundHit = true; return; }
...
LP lanePalette(LaneType t, int depth, bool night)
{
    ...
    {
    case SAFE:
        p.topBg = dk(239); p.topFg = dk(252);
        p.frontBg = dk(235); p.frontFg = dk(245);
        break;
    case GRASS:
        ...
        break;
    case ROAD:
        p.topBg = dk(241); p.topFg = dk(248);
        p.frontBg = dk(236); p.frontFg = dk(243);
        break;
    case WATER:
...
{
    switch (t) {
        case SAFE:  return Term::sfg(245) + " SAFE " + Term::sreset();
        case GRASS: return Term::sfg(34)  + "GRASS " + Term::sreset();
        case ROAD:  return Term::sfg(248) + " ROAD " + Term::sreset();
        case WATER: return Term::sfg(39)  + "WATER " + Term::sreset();
    }
    return "      ";
}
...
void buildHUD(ScreenBuf& buf, const Player& p, int hi, const std::string& name, bool paused)
{
    // Row 0: top border
    {
        ...
        for (int i = 0; i < TOTAL_W - 2; ++i) ln += Term::sbg(234) + Term::sfg(238) + "═";
    ...
    // Row 2: divider
    {
        std::string ln;
        ln += Term::sbg(234) + Term::sfg(240) + "╠";
        for (int i = 0; i < TOTAL_W - 2; ++i) ln += Term::sbg(234) + Term::sfg(237) + "═";
        ln += Term::sbg(234) + Term::sfg(240) + "╣" + Term::sreset();
        buf.lines[2] = ln;
    }
...
enum Key { K_NONE, K_UP, K_DOWN, K_LEFT, K_RIGHT,
           K_QUIT, K_RESTART, K_LEADERBOARD, K_PAUSE, K_NIGHT, K_SOUND };
...
std::string splash()
{
    ...
    for (int i = 0; i < 6; ++i) { Term::fg(colors[i]); std::cout << "  " << T1[i] << "\n"; }
    std::cout << "\n";
    for (int i = 0; i < 6; ++i) { Term::fg(colors[5-i]); std::cout << "  " << T2[i] << "\n"; }
    Term::reset_attr();
                           ...
                           int hi, const std::string& name)
{
    ...
    Term::reset_attr(); Term::canonical();
...
void runGame(const std::string& playerName)
{
    ...
    while (!quit)
    {
        ...
        if (!paused)
        {
            ...
            if (ms >= TICK_MS)
            {
                ...
                {
                    if (!player.alive) Term::beep_death();
                    else               Term::beep_hit();
                }
    ...
    padLine("Speed Tier  ", g_state.speedTier==0?"Normal":g_state.speedTier==1?"Fast":"Turbo");
